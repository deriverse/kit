...
export function vmRemoveKaminoData(tag: number): Buffer {
  let buf = Buffer.alloc(8);
  buf.writeUint8(tag, 0);
  buf.writeUint8(0, 1);
  buf.writeUint16LE(0, 2);
...
export function kaminoChangePositionData(tag: number, flags: number, instrId: number, borrowDelta: number, collateralDelta: number, customId: number): Buffer {
  let buf = Buffer.alloc(32);
  buf.writeUint8(tag, 0);
