...
function getMultipleSpotOrders(
...
function getMultiplePerpOrders(
  data: Base64EncodedDataResponse,
  firstEntry: number,
  clientId: number
): Array<OrderModel> {
...
export class Engine {
  ...
  async initialize(): Promise<boolean> {
    try {
      this.drvsAuthority = (
        await getProgramDerivedAddress({ programAddress: this.programId, seeds: ['ndxnt'] })
      )[0];
      ...
      const instrAccounts = await this.findAccountsByTag(AccountType.INSTR, {
        offset: 8,
        length: 16,
      });
  ...
  async setSigner(signer: Address<any>) {
    ...
    try {
      const info = await this.rpc
        .getAccountInfo(clientPrimaryAccount, {
          commitment: this.commitment,
          encoding: 'base64',
        })
        .send();
      if (info.value) {
        const clientPrimaryAccountHeaderModel = ClientPrimaryAccountHeaderModel.fromBuffer(
          info.value.data
        );
        if (clientPrimaryAccountHeaderModel.walletAddress == signer) {
          ...
          if (date < clientPrimaryAccountHeaderModel.refProgramExpiration) {
            ...
            let refInfo = await this.rpc
              ...
              .send();
            const refClientPrimaryAccountHeaderModel = ClientPrimaryAccountHeaderModel.fromBuffer(
              refInfo.value.data
            );
            this.refClientCommunityAccount = await this.findClientCommunityAccount(
  ...
  async getClientData(): Promise<GetClientDataResponse> {
    ...
    const infos = await this.rpc
      .getMultipleAccounts([this.clientPrimaryAccount, this.clientCommunityAccount], {
        commitment: this.commitment,
        encoding: 'base64',
      })
      .send();
    ...
    let clientCommunityAccountHeaderModel = ClientCommunityAccountHeaderModel.fromBuffer(
      infos.value[1].data
    );
    ...
    for (var i = 0; i < clientCommunityAccountHeaderModel.count; ++i) {
      const offset =
        ClientCommunityAccountHeaderModel.LENGTH + ClientCommunityRecordModel.LENGTH * i;
      let clientCommunityRecordModel = ClientCommunityRecordModel.fromBuffer(
        infos.value[1].data,
        offset
      );
      ...
      clientCommunityRecords.set(
        clientCommunityRecordModel.crncyTokenId,
        clientCommunityRecordModel
      );
    }
    const drvsDec = this.tokenDec(0);
  ...
  async getClientPerpOrders(args: GetClientPerpOrdersArgs): Promise<GetClientPerpOrdersResponse> {
    ...
    if (args.bidsCount > 1) {
      let info = await this.rpc
        .getAccountInfo(bidOrdersAccount, { commitment: this.commitment, encoding: 'base64' })
        .send();
      bidContextSlot = Number(info.context.slot);
    ...
    if (args.asksCount > 1) {
      let info = await this.rpc
        .getAccountInfo(askOrdersAccount, { commitment: this.commitment, encoding: 'base64' })
        .send();
      askContextSlot = Number(info.context.slot);
  ...
  async depositInstruction(args: DepositArgs): Promise<any> {
    ...
    const tokenProgramId =
      (token.mask & 0x80000000) != 0 ? TOKEN_2022_PROGRAM_ID : TOKEN_PROGRAM_ID;
    ...
    if (this.privateMode) {
      keys.push({
        address: await this.getAccountByTag(AccountType.PRIVATE_CLIENTS),
        role: AccountRole.WRITABLE,
      });
    }
    if (exists) {
      if (args.tokenId == 0) {
        ...
        keys.push({ address: this.clientCommunityAccount, role: AccountRole.WRITABLE });
      }
      return {
        accounts: keys,
        programAddress: this.programId,
      ...
      if (args.refId != null && args.refId != undefined) {
        ...
        keys.push({
          address: await this.findClientPrimaryAccount(args.refWallet),
          role: AccountRole.WRITABLE,
        });
        keys.push({
          address: await this.findClientCommunityAccount(args.refWallet),
          role: AccountRole.WRITABLE,
        });
      } else {
        refId = 0;
      ...
      return {
        ...
        data: depositData(
          ...
          allFunds,
          args.tokenId,
          amount * this.tokenDec(args.tokenId),
          slot,
          refId
        ),
      };
  ...
  async withdrawInstruction(args: WithdrawArgs): Promise<any> {
    ...
    if (args.spot != undefined) {
      for (var i = 0; i < args.spot.length; ++i) {
        ...
        if (
          instr.header.assetTokenId == args.tokenId ||
          instr.header.crncyTokenId == args.tokenId
        ) {
    ...
    if (args.tokenId == 0) {
      ...
      keys.push({ address: await this.clientCommunityAccount, role: AccountRole.WRITABLE });
    }
    return {
      accounts: keys,
      programAddress: this.programId,
  ...
  async newSpotOrderInstruction(args: NewSpotOrderArgs): Promise<any> {
    ...
    if (this.refClientPrimaryAccount != null && this.refClientPrimaryAccount != undefined) {
      keys.push({ address: this.refClientPrimaryAccount, role: AccountRole.WRITABLE });
      keys.push({ address: this.refClientCommunityAccount, role: AccountRole.WRITABLE });
    }
    return {
      accounts: keys,
      programAddress: this.programId,
  ...
  async spotOrderCancelInstruction(args: SpotOrderCancelArgs): Promise<any> {
    ...
    if (drvs) {
      keys.push({ address: await this.findClientCommunityAccount(), role: AccountRole.WRITABLE });
    }
    return {
      accounts: keys,
      programAddress: this.programId,
  ...
  async upgradeToPerpInstructions(args: InstrId): Promise<any[]> {
    ...
    const perpMapsAccountLamports = await this.rpc
      .getMinimumBalanceForRentExemption(BigInt(perpMapsAccountSize))
      .send();
  ...
  async perpDepositInstruction(args: PerpDepositArgs): Promise<any> {
    ...
    return {
      ...
      data: perpDepositData(
        11,
        args.instrId,
        args.amount * this.tokenDec(instr.header.crncyTokenId)
      ),
    };
  }
  ...
  async perpBuySeatInstruction(args: PerpBuySeatArgs): Promise<any> {
    ...
    const slippagePrice =
      (perpSeatReserve(instr.header.perpClientsCount + 1) -
        perpSeatReserve(instr.header.perpClientsCount)) *
      (1 + splippage);
  ...
  async newInstrumentInstructions(args: NewInstrumentArgs): Promise<any[]> {
    ...
    const tokenProgramId =
      assetInfo.value.owner == TOKEN_2022_PROGRAM_ID ? TOKEN_2022_PROGRAM_ID : TOKEN_PROGRAM_ID;
    ...
    const mapsAccountLamports = await this.rpc
      .getMinimumBalanceForRentExemption(BigInt(mapsAccountSize))
      .send();
    const createMapsAccountIx = getCreateAccountWithSeedInstruction({
      payer: this.signer,
      baseAccount: this.signer,
    ...
    let keys = [
      ...
      { address: this.rootAccount, role: AccountRole.WRITABLE },
      {
        address: await this.getTokenAccount(args.assetMint),
      ...
      { address: await this.getTokenAccount(args.crncyMint), role: AccountRole.READONLY },
      {
        address: newAssetToken
          ? args.newProgramAccountAddress
          : this.tokens.get(assetTokenId)!.programAddress,
